...
# ⑤ 🕸️ หน้าเว็บ import ครบไหม — จับ "ใช้ตัวแปรแต่ลืม import"
#    Vite build ผ่านได้ทั้งที่ตัวแปรไม่มีจริง เพราะมันพังตอน "รัน" ไม่ใช่ตอน "build"
#    13 ส.ค. 69 เว็บจริงดับเพราะเรื่องนี้ (main.jsx ใช้ WORKSHOP_LIVE โดยไม่ได้ import)
#    ทุกหน้าที่มีแถบเมนูขึ้น "โหลดหน้านี้ใหม่" ลูกค้าซื้อไม่ได้เลย และด่านเดิมจับไม่ได้
if [ -f "$TMP/scripts/check-web-imports.mjs" ]; then
  if ! ( cd "$TMP" && node scripts/check-web-imports.mjs > /tmp/pf_web.log 2>&1 ); then
    sed 's/^/  /' /tmp/pf_web.log
    echo "❌ หน้าเว็บจะพังตอนลูกค้าเปิด — ห้ามขึ้นเว็บ"; exit 1
  fi
  sed 's/^/  /' /tmp/pf_web.log
fi

# ⑤.๕ 📱 กันหน้าเว็บล้นจอมือถือ — grid ที่ย่อลงมือถือต้องใช้ minmax(0,1fr) ห้ามใช้ 1fr เปล่าๆ
#      คิมเจอเอง 21 ส.ค. 69: หน้าจองเวิร์กช็อปบนมือถือ "มันใหญ่หมดเลย" ตัวหนังสือกับปุ่มจองล้นออกนอกจอ
#      สาเหตุ: 1fr = minmax(auto,1fr) ตัว auto ยอมกว้างตามรูปปกต้นฉบับ (1200px)
#      คอลัมน์เลยบวมเป็น 1022px บนจอ 375px แล้ว body{overflow-x:clip} ตัดทิ้ง เลื่อนตามไปดูก็ไม่ได้
#      ตอนนั้นพังจริงหน้าเดียว (จองเวิร์กช็อป) แต่หน้าขายคอร์สกับหน้าเรียนเขียนแบบเดียวกันเป๊ะ
#      ที่ยังไม่พังเพราะรูปปกบังเอิญเล็กกว่าเท่านั้น — วันไหนเปลี่ยนรูปใหญ่ขึ้นก็พังทันที จึงกันไว้ทั้งหมด
BAD_GRID=$(grep -rn "grid-template-columns: *1fr *!important" "$TMP/web/src" 2>/dev/null | grep -v minmax || true)
if [ -n "$BAD_GRID" ]; then
  echo "❌ หน้าเว็บจะล้นจอบนมือถือ — ต้องเปลี่ยนเป็น minmax(0,1fr):"
  echo "$BAD_GRID" | sed "s|$TMP/|     |"
  exit 1
fi
echo "  ✅ ไม่มี grid ที่ล้นจอมือถือ"
