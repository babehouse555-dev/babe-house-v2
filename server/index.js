...
// ค่าเดินทางออกไปสอนนอกสถานที่ (คิมส่งราคาทางการมา 17 ส.ค. 2569)
//   กรุงเทพฯ = +2,000 บาทตายตัว
//   ต่างจังหวัด = ค่าเดินทาง+ที่พัก "ตามจริง" ไม่มีค่าคงที่ → ใส่ 0 แล้วบอกในหมายเหตุแทน
//   ⛔ ห้ามเดาเป็นตัวเลข ขึ้นกับจังหวัดและวันเดินทาง เดาแล้วผิดแน่ และผิดในทางที่ลูกค้าเสียเปรียบ
const CORP_TRAVEL = { "ที่บริษัท (กรุงเทพฯ / ปริมณฑล)": 200000, "ที่บริษัท (ต่างจังหวัด)": 0, "ที่ Babe House": 0, "ออนไลน์": 0 };
const CORP_TOPICS = [
...
function corpQuote(headcount, place) {
  ...
  const total = tier.satang + travel;
  return { headcount: head, tier_label: tier.label, base_satang: tier.satang, travel_satang: travel,
    total_satang: total, upcountry,
    // 💡 เฉลี่ยต่อคน — ตัวเลขที่ทำให้ฝ่ายบุคคลเห็นภาพว่าคุ้ม (คิมใช้ตัวเลขนี้ตอนเสนอราคาอยู่แล้ว)
    //    ปัดลงหลักร้อยให้ตรงสไตล์ที่คิมเขียน (30 คน = "เฉลี่ย 3,300/คน" ไม่ใช่ 3,333.33)
    //    ปัด "ลง" เสมอ ไม่ปัดขึ้น — ตัวเลขที่โฆษณาต้องไม่สูงกว่าที่ลูกค้าจ่ายจริง
    per_head_satang: Math.floor(tier.satang / head / 10000) * 10000,
    note: upcountry ? "ต่างจังหวัดคิดค่าเดินทางและค่าที่พักตามจริง เราจะแจ้งยอดชัดเจนในใบเสนอราคาค่ะ" : null };
}
...
const invoiceEmailHtml = (r) => {
  ...
  return wrap(
    ...
    `</table><br>` +
    btn(`${appBaseUrl()}/invoice/${r.invoice_id}`, "เปิด / ดาวน์โหลดใบเสร็จ") + `<br>` +
    // ⚠️ ต้องบอกไว้ตรงนี้ — กดปุ่มแล้วเจอหน้า "เข้าสู่ระบบก่อนนะคะ" ทันที
    //    ถ้าไม่เตือน ลูกค้าจะนึกว่าลิงก์เสียแล้วเลิกกลางทาง = กลับไปเป็นปัญหาเดิมที่เราเพิ่งแก้
    `<span style="font-size:13px;color:#6b6b76">กดแล้วระบบจะขอให้เข้าสู่ระบบด้วยอีเมลนี้ก่อนนะคะ เพื่อความปลอดภัยของข้อมูล — ใช้รหัส 6 หลักที่ส่งเข้าเมลนี้ค่ะ</span><br><br>` +
...
// ⛔ คอร์สที่ปิดการขายด้วยมือ — ยังไม่ลบทิ้ง เพราะคนที่ซื้อไปแล้วต้องเข้าเรียนได้ตลอด
//    (/api/academy/learn ไม่เช็ครายการนี้ เช็คแค่ว่าซื้อแล้วจริงไหม → สิทธิ์เดิมไม่หาย)
// 53 = Ai Studio — คิมสั่งปิด 20 ส.ค. 69: "เนื้อหาไม่ค่อยอัปเดต"
//      ลูกค้าจ่ายเต็มราคา ฿3,745 แต่ได้ของที่ล้าสมัย = ไม่ควรขายต่อจนกว่าจะถ่ายใหม่
//      ปลดออกจาก Set นี้เมื่อไหร่ก็กลับมาขายได้ทันที ไม่ต้องแก้อย่างอื่น
const HIDDEN_COURSES = new Set(["15", "21", "40", "41", "42", "53"]);
...
async function seedTeamIfEmpty() {
  ...
  const T = [
    ...
    // 🧑‍💻 bense = ฟรีแลนซ์ที่ทำได้ทั้งคอนเทนต์ Academy และรับงานตัดต่อฝั่งโปรดักชั่น (คิมยืนยัน 13 ส.ค.)
    //    โควตา 0 = ระบบไม่ยัดงานให้ ต้องกดเปิดวันเอง เหมือน NIN (ฟรีแลนซ์ทุกคนเป็น opt-in)
    ["bense", "bense", "editor", "ตัดต่อ + คอนเทนต์ (ฟรีแลนซ์)", "both", 0],
...
async function pickAssignee(job, excludeIds = []) {
  ...
  // 🙋 ใครก็รับงานเสริมได้ ขอแค่ "ติ๊กว่าว่าง" เอง (คิมสั่ง 13 ส.ค.)
  //    เดิมตัด AE (Luktarn) ออกตายตัว เพราะ 7 ส.ค. คิมบอกว่าให้เป็นคนตรวจอย่างเดียว
  //    ตอนนี้เปิดให้รับเสริมได้ แต่ยังปลอดภัย เพราะเงื่อนไข capacity > 0 บังคับอยู่แล้วว่า
  //    ต้องมีการเปิดวันไว้เองเท่านั้น (โควตาประจำวันของ AE ถูกตั้งเป็น 0 เสมอ)
  //    → ไม่เปิดวัน = ไม่มีวันได้งาน ระบบไม่ยัดให้เด็ดขาด
