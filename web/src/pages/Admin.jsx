  ...
  return (
    <div className="wrap page-pad">
      ...
      <div className="card"><div className="between"><h3>ข้อมูลนักเรียน ({students.length}){filter && <span className="muted"> · กรอง: {filter}</span>}</h3><div className="row">{filter && <button className="link" onClick={() => loadStudents(null)} style={{ background: "none", border: 0 }}>ดูทั้งหมด</button>}<button className="btn ghost" onClick={csv} style={{ padding: "8px 14px" }}>⬇ CSV</button></div></div>
        <div className="scroll" style={{ marginTop: 12 }}><table><thead><tr><th>วันที่</th><th>อีเมล</th><th>IG</th><th>ธุรกิจ</th><th>อุตสาหกรรม</th><th>เป้าหมาย</th><th>เล่ม</th></tr></thead>
          <tbody>{students.length === 0 ? <tr><td colSpan={7} className="muted">ยังไม่มีข้อมูล</td></tr> : students.map((s, i) => <tr key={i}><td>{String(s.created_at || "").slice(0, 10)}</td><td>{s.email}</td><td>{s.instagram_account}</td><td>{s.business_type}</td><td>{s.industry || "-"}</td><td>{(s.monthly_goal || "").slice(0, 40)}</td><td>{s.blueprint_id && s.user_id ? <a className="link" target="_blank" href={`/dashboard?user_id=${encodeURIComponent(s.user_id)}&billing_cycle=${encodeURIComponent(s.billing_cycle)}&blueprint_id=${encodeURIComponent(s.blueprint_id)}`}>เปิด ›</a> : "-"}</td></tr>)}</tbody>
        </table></div>
      </div>
    </div>
  );
