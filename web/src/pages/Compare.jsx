...
export default function Compare() {
  ...
  if (months.length < 2) return <div className="wrap narrow page-pad"><h1 className="page">เส้นทางการเติบโต</h1><div className="card center muted">ต้องมีอย่างน้อย 2 เดือนถึงจะเทียบได้ค่ะ<br /><br /><Link className="btn" to="/account">กลับบัญชีของฉัน</Link></div></div>;
  ...
  return (
    <div className="wrap narrow page-pad">
      ...
      <div className="card"><h3 style={{ marginBottom: 14 }}>เทียบเดือนแรก ↔ เดือนล่าสุด</h3>
        <table><thead><tr><th>ตัวชี้วัด</th><th style={{ textAlign: "right" }}>{first.billing_cycle.replace("_", " ")}</th><th style={{ textAlign: "right" }}>{last.billing_cycle.replace("_", " ")}</th><th style={{ textAlign: "right" }}>เปลี่ยน</th></tr></thead>
