...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    api(latestUrl, { token: session.token, adminKey: localStorage.getItem("babe_admin_key") || undefined })
      .then(d => { setBp(d.blueprint); setUploaded(new Set(d.marathon || [])); setStartedAt(d.started_at || null); setImproveCount(d.improve_count || 0); const ready = d.content_status === "ready"; setContentReady(ready); if (ready) setTab("calendar"); /* นักเรียนเก่ากลับมา = เห็นแผน 30 วัน (มีภาพรวมอยู่หัวแท็บ) */ })
    ...
    <div>
      ...
      <div className="wrap page-pad" style={{ maxWidth: 820 }}>
        ...
        {!demo && <div className="card center" style={{ background: "linear-gradient(135deg,#EAF3FD,#F4F9FF)", border: "1px solid #d6e7fa", marginTop: 24 }}>
          ...
          {/* 🔭 ทางเข้าหน้าเทียบการเติบโต — เดิมมีลิงก์แค่ในหน้าบัญชี ลูกค้าเดือนแรกแทบไม่มีทางเจอ */}
          <div style={{ marginTop: 12 }}>
            <Link className="link" to={`/compare?channel=${encodeURIComponent(bp.instagram_account || "")}`} style={{ fontSize: 13.5 }}>
              {t("db_peek_growth")}
            </Link>
          </div>
        </div>}
